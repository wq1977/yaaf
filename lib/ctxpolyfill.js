module.exports = async (ctx, next) => {
  ...
  if ('x-session' in ctx.req.headers) {
      ...
      ctx.info = ctx.log.info.bind(this.log, ctx.session);        
      ctx.error = ctx.log.error.bind(this.log, ctx.session);                
      ctx.warn = ctx.log.warn.bind(this.log, ctx.session);                
  }
  ctx.errorCode = require('./errorCode')
  ctx.func = require('./func')
  ctx.db = require('./mysql')(ctx.config.mysql.url)
  ctx.cache = require('./redis')(ctx.config.redis.url)
  await next();
}